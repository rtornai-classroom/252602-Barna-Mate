...
GLchar windowTitle[] = "Barna Mate";

// Kamera paraméterek (Hengerfelület) 
float cameraR = 9.0f;     // Sugár
float cameraAngle = 0.0f; // Körbejárás szöge
float cameraZ = 0.0f;     // Magasság a Z-tengelyen
bool lightOn = true; 
...
void framebufferSizeCallback(GLFWwindow* window, int width, int height) {
    ...
    matProjection = glm::perspective(glm::radians(55.0f), (float)width / height, 0.1f, 100.0f); // 55 fok [cite: 146]
}

int main() {
    init(3, 3, GLFW_OPENGL_CORE_PROFILE);

    std::cout << "[<]/[>]:  Kamera forgatasa Z-tengely korul" << std::endl;
    std::cout << "[^]/[v]:  Kamera mozgatasa a Z-tengely menten" << std::endl;
